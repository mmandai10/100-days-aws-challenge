...
**テーマ:** ALB + ECS Fargate 連携、Auto Scaling

**完了したこと:**

1. **ALB + ECS Fargate 連携を Terraform で構築**
   - ALB（Application Load Balancer）を ECS Fargate サービスと連携
   - ALB DNS 名経由でアクセス確認成功
...
3. **セキュリティグループ分離**
   - ALB 用セキュリティグループ（外部からの HTTP トラフィック許可）
   - ECS 用セキュリティグループ（ALB からのトラフィックのみ許可）
   - セキュリティの多層防御を実践

4. **Terraform リソース管理**
   - 全 24 リソースを Terraform で管理
   - 学習後に `terraform destroy` で全リソース削除済み

**作成したファイル:**
- `projects/container-basics/day53-alb/terraform/` 配下
...
**学んだこと:**
- ALB はリクエストを複数の ECS タスクに分散
- Target Tracking は指定メトリクスを維持するようタスク数を自動調整
- セキュリティグループ分離で ECS タスクへの直接アクセスを防止
- Terraform で複数リソースの依存関係を宣言的に管理
