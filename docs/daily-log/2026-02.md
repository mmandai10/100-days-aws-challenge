...
3. **セキュリティグループ分離設計**
   ...
   - 分離の理由: 攻撃面の最小化、責務の明確化

4. **ECS Cluster + Service + Task Definition**
   - Day 51 の ECR イメージ（day51-sample-app）を使用
   - Fargate: CPU 256, Memory 512
   - CloudWatch Logs: /ecs/day53-alb（保持 1 日）
   - Service が ALB Target Group に登録される

5. **Auto Scaling（AWS CLI で設定）**
...
Internet → ALB (HTTP:80) → ECS Fargate Tasks (port 3000)
           ...
           subnet-1c         Auto Scaling: 1-4 tasks
```

**学んだこと:**
- ALB は最低 2 AZ のサブネットが必要
- Fargate の Target Group は target_type = "ip" が必須
- セキュリティグループ参照（referenced_security_group_id）で SG 間の関係を定義
- AWS Academy の IAM 制限は Terraform の代わりに CLI で回避可能
- ECS Service の depends_on で ALB Listener 作成完了を待つ
