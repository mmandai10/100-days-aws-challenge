...
**テーマ:** ECS Fargate + ALB 連携 + Auto Scaling

**完了したこと:**

1. **ALB の必要性を理解（Day 52 の課題から）**
   - Day 52: Public IP 直接アクセス → タスク再起動で IP 変更、単一障害点
   - ALB が解決すること: 固定 DNS 名、負荷分散、障害時の自動切替
   - ALB コンポーネント: Listener（Port 80）→ Target Group（Port 3000）→ Tasks

2. **VPC + マルチ AZ 構成を Terraform で構築**
   - VPC: 10.0.0.0/16
   - Public Subnet x2: ap-northeast-1a (10.0.1.0/24), 1c (10.0.2.0/24)
   - Internet Gateway + Route Table
   - ALB は最低 2 AZ 必要 → 2 サブネット作成

3. **ALB（Application Load Balancer）構成**
   - ALB: day53-alb-alb（外部公開、application タイプ）
   - Target Group: IP ターゲット（Fargate 必須）、ポート 3000
   - ヘルスチェック: /health パス、30 秒間隔
   - Listener: HTTP:80 → Target Group へフォワード

4. **セキュリティグループ分離設計**
   - ALB SG: インターネットから HTTP:80 を許可
   - ECS SG: ALB SG からのみ port 3000 を許可（referenced_security_group_id）
   - ECS タスクに直接アクセス不可 → ALB 経由のみ
   - 分離の理由: ALB を「門番」にして攻撃面を最小化

5. **ECS Cluster + Service + Task Definition**
   - Day 51 の ECR イメージ（day51-sample-app:latest）を使用
   - Fargate: CPU 256, Memory 512
   - CloudWatch Logs: /ecs/day53-alb（保持 1 日）
   - Service が ALB Target Group にタスクを自動登録

6. **Auto Scaling（AWS CLI で設定）**
   - AWS Academy 制限: application-autoscaling:ListTagsForResource が不許可
   - → Terraform 管理不可、CLI で直接設定
   - Scalable Target: min 1 / max 4
   - Target Tracking Policy: CPU 50% で自動スケール
   - Cooldown: ScaleIn/ScaleOut ともに 60 秒

7. **デプロイ検証**
   - ALB DNS: `http://day53-alb-alb-48214651O.ap-northeast-1.elb.amazonaws.com`
   - レスポンス: `{"message":"Hello from ECR!","version":"1.0.0","environment":"production"}`
   - ヘルスチェック: `{"status":"healthy","timestamp":"2026-02-06T05:00:42.076Z"}`
   - Terraform リソース: 24 個作成
   - 検証後 `terraform destroy` で全リソース削除（ALB は時間課金）

8. **ECS 階層構造の復習**
   - Cluster = オフィスビル（環境単位）
   - Service = テナント/会社（アプリ単位、Task 数を維持）
   - Task = 従業員（Task Definition から起動した実体）
   - Container = 従業員の担当業務（1 Task に複数可能）
   - Day 53 構成: 1 Task = 1 Container（day51-sample-app）
   - マルチコンテナ Task: サイドカーパターン（app + log-agent + proxy が localhost/storage 共有）

9. **学習モード vs 実装モードの振り返り**
   - Day 52: 手動ステップバイステップ → 深い理解
   - Day 53: Claude Code 自動化 → 効率的だが理解が浅くなるリスク
   - 方針策定: 新概念 = 学習モード、既知パターン = 実装モード

**作成したファイル:**
- `projects/container-basics/day53-alb/terraform/main.tf`（VPC, Subnets, IGW, RT）
- `projects/container-basics/day53-alb/terraform/alb.tf`（ALB, TG, Listener）
- `projects/container-basics/day53-alb/terraform/ecs.tf`（Cluster, Task Def, Service）
- `projects/container-basics/day53-alb/terraform/security.tf`（SG 分離）
- `projects/container-basics/day53-alb/terraform/autoscaling.tf`（CLI 手順コメント）
- `projects/container-basics/day53-alb/terraform/iam.tf`（Task Execution Role）
- `projects/container-basics/day53-alb/terraform/variables.tf`
- `projects/container-basics/day53-alb/terraform/outputs.tf`

**アーキテクチャ:**
```
Internet → ALB (HTTP:80) → ECS Fargate Tasks (port 3000)
           [ALB SG]         [ECS SG: ALB からのみ]
           subnet-1a         subnet-1a, 1c
           subnet-1c         Auto Scaling: 1-4 tasks (CPU 50%)
```

**理解度チェック（セッション内で回答）:**
- Q: なぜ ALB と ECS の SG を分けるか？ → ALB を門番にして直接アクセスを遮断
- Q: なぜ target_type = "ip" か？ → Fargate は EC2 インスタンスがなく、Task の IP で登録
- Q: Listener の default_action とは？ → Port 80 へのリクエストを Target Group に転送するルール

**ワークフロー方針（今後適用）:**
- Claude.ai: 概念説明、設計相談、ログ更新、Git 操作
- Claude Code: ファイル作成・編集・エラー解析のみ
- PowerShell: terraform / aws CLI / curl 等のコマンド実行（トークン節約）
- 学習モード（新概念）: Claude.ai で概念理解 → 自分で Terraform 記述 → PS で実行
- 実装モード（既知パターン）: Claude Code でファイル作成 → PS で実行

**その他:**
- CLAUDE.md 作成: Claude Code のセッション間コンテキスト用（claude-mem は /plugin 未対応で保留）
...
**テーマ:** ECS デプロイ戦略（ローリングアップデート実践）

**完了したこと:**

1. **デプロイ戦略の理論**
   - ローリングアップデート: 新旧タスクを段階的に入れ替え（ECS デフォルト）
   - Blue/Green: 新環境を丸ごと作って ALB で一括切替（CodeDeploy 連携）
   - カナリア: トラフィックの一部だけ新バージョンに流す

2. **ローリングアップデートのパラメータ**
   - `deployment_minimum_healthy_percent = 100`: 更新中も最低 100% タスク稼働
   - `deployment_maximum_percent = 200`: 最大 200% まで同時存在可能
   - desired_count=1 の場合: OLD 1 + NEW 1 = 最大 2 タスク
   - desired_count=2 の場合: OLD 2 + NEW 2 = 最大 4 タスク

3. **実践: 1 タスクでのローリングアップデート**
   - v1 イメージを ECR にプッシュ
   - Terraform で ECS + ALB 環境構築
   - v2 イメージに更新 → ダウンタイムゼロで切替わり確認
   - hostname が変わることで物理的に別タスクへの切替を実証

4. **実践: 2 タスクでのローリングアップデート**
   - desired_count=2 に変更、v1 x 2 で安定稼働
   - v2 に更新 → 約 40 秒間 v1 と v2 が混在
   - ALB がラウンドロビンで振り分け → ユーザーによって v1/v2 が返る
   - ダウンタイムゼロ（503 は一度もなし）

5. **現場での使い分け**
   - 日常のデプロイの 90% 以上はローリング
   - 大規模改修でも Expand and Contract パターンで分割すればローリングで対応可能
   - Blue/Green は「分割できない変更」のみ（ランタイム変更、規制対応等）

**観察結果（2 タスク）:**
```
05:12  v2  ip-10-0-1-66   ← NEW タスク 1 が参加
05:17  v1  ip-10-0-1-36   ← OLD タスク 1 まだ稼働
05:23  v2  ip-10-0-2-79   ← NEW タスク 2 も参加（4 タスク同時存在）
05:25  v1  ip-10-0-2-230  ← v1 と v2 が混在
05:54  v1  ip-10-0-2-230  ← 最後の v1 レスポンス
05:56  v2  ip-10-0-2-79   ← 以降 v2 のみ。完了
```

**作成したファイル:**
- `projects/container-basics/day54-rolling-update/app/server.js`
- `projects/container-basics/day54-rolling-update/app/package.json`
- `projects/container-basics/day54-rolling-update/app/Dockerfile`
- `projects/container-basics/day54-rolling-update/terraform/main.tf`
- `projects/container-basics/day54-rolling-update/terraform/alb.tf`
- `projects/container-basics/day54-rolling-update/terraform/ecs.tf`（ローリングパラメータ明示）
- `projects/container-basics/day54-rolling-update/terraform/security.tf`
- `projects/container-basics/day54-rolling-update/terraform/iam.tf`
- `projects/container-basics/day54-rolling-update/terraform/variables.tf`
- `projects/container-basics/day54-rolling-update/terraform/outputs.tf`

**学んだこと:**
- ローリングアップデートは新旧混在が発生する → 後方互換が必須
- Expand and Contract パターン: 大規模改修でも分割すればローリングで対応可能
- 2 タスクで約 40 秒間 v1/v2 混在を実測
- terraform destroy のエラーハンドリングは Claude Code 向き
- ツールの使い分け: 学習=Claude.ai、エラー対応サイクル=Claude Code
...
**テーマ:** Blue/Green デプロイ + Container Insights

### Blue/Green デプロイ（概念 + 実践）

**ローリング vs Blue/Green:**
- ローリング: 1つの TG 内で新旧段階的入替。Day 54 では約40秒間混在
- Blue/Green: 2つの TG（Blue=現行、Green=新版）を ALB Listener で切替。混在ゼロ
- 実測: v1 → v2 の切替が約2秒で完了、新旧混在なし

**Blue/Green の構成要素:**
- Target Group 2つ（Blue + Green）
- Listener 2つ（本番:80 + テスト:8080）
- CodeDeploy（Application + Deployment Group）
- appspec.json（デプロイ指示書）
- ECS Service の deployment_controller = CODE_DEPLOY

**使い分け:**
- 日常デプロイ（90%以上）: ローリング
- 後方互換が保てない変更: Blue/Green
- 規制対応（金融、医療）: Blue/Green

**Expand and Contract パターン:**
- 破壊的変更を2ステップに分割（Expand: 新旧両対応 → Contract: 古いコード削除）

### Container Insights

**ECS 標準 vs Container Insights:**
- 標準: Service 全体の平均値、無料、Auto Scaling はこれで動く
- Container Insights: タスクファミリー/タスク単位、障害調査用、有料

**確認できたこと:**
- /stress エンドポイントで CPU スパイクを観察
- タスクファミリービューで個別メトリクス確認
- Terraform での有効化: `setting { name = "containerInsights" value = "enabled" }`

### Auto Scaling の反応時間

- スパイク発生から新タスク稼働まで約3〘5分
- 対策: 余裕を持ったタスク数、スケジュールベース、低めの閾値、CDN

### トラブルシューティング

- CodeDeploy 権限問題: IAM 反映遅延（結果整合性）でインラインポリシー追加後も数分待ちが必要
- v2 イメージ起動失敗: PowerShell `-replace` + `Set-Content` で日本語コメントが文字化け、改行崩壊
- EC2/IAM 削除エラー: `ec2:*` と IAM ロール操作権限をインラインポリシーで追加して解決

### ロードマップ更新

- v6.0 → v6.1: Phase 7 Project 2 に CloudFront + WAF 追加（SAA 試験範囲）

### ファイル

- `day55-container-insights/app/` - server.js, Dockerfile, package.json
- `day55-container-insights/terraform/` - Container Insights 環境
- `day55-container-insights/terraform-bluegreen/` - Blue/Green 環境（CodeDeploy 付き）
