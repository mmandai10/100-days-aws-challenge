...
### Wizardry Schema v14.0 - 施設ベースUI（Finding Hermit Nilda風）

**進捗:**

1. **施設ベースUIへの大幅リニューアル**
   - メイン画面を「リルガミンの街」として構成
   - 7施設をカードメニューとして表示
   - クリックで各施設の詳細画面へ遷移
...
3. **UI/UXの改善**
   - 施設カードにホバーエフェクト（浮き上がり + 光沢アニメーション）
   - グラデーション背景で施設ごとの雰囲気を演出
   - パーティサマリーをメイン画面に常時表示
   - 階層的ナビゲーション（街 → 施設 → 詳細）
   - ⬅️ボタンで街へ戻る
...
**技術的変更:**
- 新State: `currentFacility` - 施設画面管理
- FACILITIES配列で施設データを定義
- CSS: `.facility-card` ホバーエフェクト
- CSS: 光沢アニメーション（`::before`疑似要素）

**参考:**
- Finding Hermit Nilda（放置型RPG）のUI設計
  - 施設ベースのメニュー構成
  - カード型の視覚的なナビゲーション

**ファイル更新:**
- `projects/game/wizardry-schema.html` (v14.0)

**学び:**
- 施設ベースUIはゲームの世界観を強化する
- カードメニューは視覚的に分かりやすく、タップ/クリックしやすい
- 階層的ナビゲーションで情報過多を防ぐ
