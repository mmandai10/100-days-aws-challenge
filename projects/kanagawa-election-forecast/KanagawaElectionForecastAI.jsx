...
// AI更新機能
async function fetchAIAnalysis(districts) {
  // Web検索で最新情報を取得
  const searchResponse = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ 
        role: "user", 
        content: "2026年衆議院選挙 神奈川県 最新情勢を教えてください。特に注目選挙区や世論調査の動向について。" 
      }],
  ...
  const analysisResponse = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ 
        role: "user", 
        content: `以下の最新情報を基に、神奈川県の衆議院選挙予測を更新してください。
...
{
  "summary": "全体の情勢サマリー（100字程度）",
  "updates": [
    { "id": 選挙区番号, "prediction": "自民" or "中道", "intensity": "情勢", "analysis": "分析コメント" }
  ],
  "sources": ["参照した情報源"]
}

予測変更がない選挙区はupdatesに含めないでください。` 
      }]
    })
  });
...
export default function KanagawaElectionForecast() {
  ...
  const handleAIUpdate = async () => {
    ...
    try {
      const result = await fetchAIAnalysis(districts);
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white font-sans">
      ...
      {/* AI分析サマリー */}
      {aiSummary && (
        <div className="bg-blue-900/30 border-b border-blue-800 px-4 py-3">
          <div className="max-w-4xl mx-auto">
            <p className="text-sm text-blue-200">{aiSummary}</p>
            {aiSources.length > 0 && (
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              <div className="bg-gray-800 rounded-lg p-2 sm:p-4">
                <p className="text-gray-400 text-xs">小選挙区</p>
                <div className="flex items-baseline gap-0.5 sm:gap-1 mt-1 flex-wrap">
                  <span className="text-xl sm:text-2xl font-bold text-red-500">{jiminSeats}</span>
                  <span className="text-xs text-gray-500">自</span>
                  <span className="text-gray-500">:</span>
                  <span className="text-xl sm:text-2xl font-bold text-blue-500">{chudoSeats}</span>
                  <span className="text-xs text-gray-500">中</span>
                </div>
              </div>
              <div className="bg-gray-800 rounded-lg p-2 sm:p-4">
                <p className="text-gray-400 text-xs">比例南関東</p>
                <p className="text-xl sm:text-2xl font-bold mt-1">23<span className="text-xs sm:text-sm text-gray-500">席</span></p>
              </div>
              <div className="bg-gray-800 rounded-lg p-2 sm:p-4">
                <p className="text-gray-400 text-xs">接戦区数</p>
                <p className="text-xl sm:text-2xl font-bold mt-1 text-yellow-500">
            ...
            <div className="bg-gray-800 rounded-lg p-3 sm:p-4">
              ...
              <div className="flex h-8 rounded overflow-hidden">
                <div 
                  className="bg-red-600 flex items-center justify-center text-xs sm:text-sm font-medium"
                  style={{ width: `${(jiminSeats / totalSeats) * 100}%` }}
                >
                  自民{jiminSeats}
                </div>
                <div 
                  className="bg-blue-600 flex items-center justify-center text-xs sm:text-sm font-medium"
              ...
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-1.5 sm:gap-2">
                {districts.map((d) => (
                  <button
                    ...
                    className={`p-1.5 sm:p-2 rounded text-center transition-all hover:scale-105 ${
                      d.prediction === '自民' ? 'bg-red-900 hover:bg-red-800' :
                      'bg-blue-900 hover:bg-blue-800'
                    } ${selectedDistrict?.id === d.id ? 'ring-2 ring-white' : ''}`}
                  >
                    <p className="font-bold text-xs sm:text-sm">{d.id}</p>
                    <p className="text-xs text-gray-300 hidden sm:block">{d.prediction}</p>
                  </button>
                ))}
              </div>
      ...
      <footer className="bg-gray-800 mt-6 py-4 px-4 text-center text-gray-500 text-xs">
        ...
        <p className="mt-1">更新: {lastUpdate || '2026年2月2日'}</p>
      </footer>
    </div>
  );
}
